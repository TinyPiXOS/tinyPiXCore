#ifndef _TP_SOCKET_NOTIFIER_NEW_H_
#define _TP_SOCKET_NOTIFIER_NEW_H_
...
public:
	...
	/// @param rwCb 回调
	/// @param hangupCb 异常回调
	TpSocketNotifierNew(TpEventLoop *loop, int fd, Type type, std::function<void()> rwCb, std::function<void()> hangupCb = std::function<void()>());
	~TpSocketNotifierNew();
