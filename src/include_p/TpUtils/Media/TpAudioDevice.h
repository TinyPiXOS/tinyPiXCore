...
//获取播放速度
float Media_Get_Speed(struct MediaParams *conf)  __attribute__((used));

//设置播放速度
int Media_Set_Speed(struct MediaParams *conf,float speed)  __attribute__((used));

//内部获取用户设置的播放位置
int Media_Get_Position_S(struct MediaParams *conf)  __attribute__((used));

//内部设置实时播放位置
int Media_Set_Position_N(struct MediaParams *conf,int32_t position)  __attribute__((used));

//设置位置
int Media_Set_Position(struct MediaParams *conf,int32_t position)  __attribute__((used));

//获取位置，还没有写
int Media_Get_Position(struct MediaParams *conf)  __attribute__((used));

//暂停播放
int Media_Set_Suspend(struct MediaParams *conf) __attribute__((used));

//继续播放
int Media_Set_Continue(struct MediaParams *conf) __attribute__((used));

int Media_Get_State(struct MediaParams *conf) __attribute__((used));

//开始播放
int Media_Set_Start(struct MediaParams *conf,const char *file) __attribute__((used));

//停止播放,只是停止不会关闭声卡，不同于暂停，停止会清空大多数播放信息
int Media_Set_Stop(struct MediaParams *conf) __attribute__((used));
...
//添加播放的文件
int Media_Add_File(struct MediaParams *conf, const char *file) __attribute__((used));

//删除播放文件
int Media_Del_File(struct MediaParams *conf, const char *file) __attribute__((used));
