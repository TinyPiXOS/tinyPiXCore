...
TpString TpAppInstall::getPackVersion() // 获取安装包版本
{
    ...
    version = TpString::number(appData->conf.appConf.version.x) + "." +
              TpString::number(appData->conf.appConf.version.y) + "." +
              TpString::number(appData->conf.appConf.version.z);
...
int TpAppInstall::remove(TpString &uuid)
{
    ...
    if (is_valid_uuid(uuid.c_str()) != 1)
        return -1;

    appid.value = uuid;

    if (!(find_directory(APP_INSTALL_PATH, appid.value.c_str()) > 0))
    {
