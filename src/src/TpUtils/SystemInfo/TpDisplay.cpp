...
static double caculateDpi()
{
    ...
    tpInt32 pxHeight = caculateDisplay.resolutionHeight();
    tpInt32 pxWidth = caculateDisplay.resolutionWidth();

    // 求对角线尺寸 英寸
    tpInt32 physicsHeight = caculateDisplay.physicsHeight();
    tpInt32 physicsWidth = caculateDisplay.physicsWidth();
...
// 获取屏幕参数列表
TpList<TpDisplay::TpDisplayModeParam> TpDisplay::displayMode()
{
...
tpInt32 TpDisplay::physicsHeight()
{
    /*TpDisplayInfoParam *drm = static_cast<TpDisplayInfoParam*>(data_);
...
tpInt32 TpDisplay::physicsWidth()
{
    /*TpDisplayInfoParam *drm = static_cast<TpDisplayInfoParam*>(data_);
    if (!drm || !drm->connector) return 0;
...
tpInt32 TpDisplay::resolutionHeight()
{
    /*TpDisplayInfoParam *drm = static_cast<TpDisplayInfoParam*>(data_);
...
tpInt32 TpDisplay::resolutionWidth()
{
    /*TpDisplayInfoParam *drm = static_cast<TpDisplayInfoParam*>(data_);
...
tpInt32 TpDisplay::wmResolutionWidth()
{
    TpConfig conf(TINYPIX_CONF_PATH);
    return conf.value("display-setting/width").toInt();
}

tpInt32 TpDisplay::wmResolutionHeight()
{
    TpConfig conf(TINYPIX_CONF_PATH);
    return conf.value("display-setting/height").toInt();
}

TpSize TpDisplay::wmResolutionSize()
{
    TpConfig conf(TINYPIX_CONF_PATH);
    return TpSize(conf.value("display-setting/width").toInt(), conf.value("display-setting/height").toInt());
}

tpInt32 TpDisplay::wmPhysicsHeight()
{
    tpInt32 physics = physicsHeight();
    tpInt32 TpWMPhysics = wmResolutionHeight();
    tpInt32 resolution = resolutionHeight();

    if (physics < 0 || TpWMPhysics < 0 || resolution <= 0)
        return -1;
    double TpWMResolution = (double)physics * (double)TpWMPhysics / (double)resolution;
    return (tpInt32)TpWMResolution;
}

tpInt32 TpDisplay::wmPhysicsWidth()
{
    tpInt32 physics = physicsWidth();
    tpInt32 TpWMPhysics = wmResolutionWidth();
    tpInt32 resolution = resolutionWidth();
