...
public:
	TpVideoFrame(){};
	TpVideoFrame(uint8_t** datas, int* lines, TpSize size, void *user_data):
		datas_(datas), lines_(lines), size_(size), userdata_(user_data){};
	~TpVideoFrame(){};
...
public:
	/// @brief 获取视频帧尺寸
	/// @return 视频帧尺寸
	TpSize size(){return size_;};
	/// @brief 获取视频帧数据，只有一行取datas_[0]，多行则取对应行数
	/// @return 视频帧数据
	uint8_t** data(){return datas_;};
