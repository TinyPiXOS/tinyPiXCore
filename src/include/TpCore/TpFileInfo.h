...
public:
    ...
    /// @brief 获取文件所在的绝对目录路径
    /// @return 文件所在的绝对目录路径
    static TpString absolutePath(const TpString &file);

    /// @brief 获取文件名部分（不含目录）
    /// @return 文件名部分
    static TpString fileName(const TpString &file);

    /// @brief 获取基本名称（不含扩展名）
    /// @return 文件基本名称
    static TpString baseName(const TpString &file);

    /// @brief 获取文件大小（字节数）
    /// @return 文件大小（字节数）
    static uint64_t size(const TpString &file);
