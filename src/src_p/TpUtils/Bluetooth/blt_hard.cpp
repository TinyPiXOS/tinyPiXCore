...
int bluet_get_adapters(BluetoothAdapterCallback callback, void *user_data)
{
    ...
    // 分配内存存储设备列表
    dl = static_cast<hci_dev_list_req *>(malloc(HCI_MAX_DEV * sizeof(struct hci_dev_req) + sizeof(uint16_t)));
    if (!dl)
    {
...
{
    scan->priv->callback_add_data->adapter_object_path = (gchar*)adapter_get_dbus_object_path(scan->priv->adapter);
...
{
    scan->priv->callback_remove_data->adapter_object_path = (gchar*)adapter_get_dbus_object_path(scan->priv->adapter);
