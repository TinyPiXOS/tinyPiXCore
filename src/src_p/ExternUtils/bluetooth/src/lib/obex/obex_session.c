...
void obex_session_set_property(ObexSession *self, const gchar *name, const GVariant *value, GError **error)
{
	g_assert(OBEX_SESSION_IS(self));
	g_assert(self->priv->properties != NULL);
	g_proxy_properties_set(self->priv->properties, OBEX_SESSION_DBUS_INTERFACE, name, value, error);
}
...
guint8 obex_session_get_channel(ObexSession *self, GError **error)
{
	g_assert(OBEX_SESSION_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_SESSION_DBUS_INTERFACE, "Channel", error);
	if(prop == NULL)
		return 0;
...
const gchar *obex_session_get_destination(ObexSession *self, GError **error)
{
	g_assert(OBEX_SESSION_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_SESSION_DBUS_INTERFACE, "Destination", error);
	if(prop == NULL)
		return NULL;
...
const gchar *obex_session_get_root(ObexSession *self, GError **error)
{
	g_assert(OBEX_SESSION_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_SESSION_DBUS_INTERFACE, "Root", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
...
const gchar *obex_session_get_source(ObexSession *self, GError **error)
{
	g_assert(OBEX_SESSION_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_SESSION_DBUS_INTERFACE, "Source", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
...
const gchar *obex_session_get_target(ObexSession *self, GError **error)
{
	g_assert(OBEX_SESSION_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_SESSION_DBUS_INTERFACE, "Target", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
