...
void obex_transfer_set_property(ObexTransfer *self, const gchar *name, const GVariant *value, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	g_proxy_properties_set(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, name, value, error);
}
...
const gchar *obex_transfer_get_filename(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Filename", error);
	if(prop == NULL)
		return NULL;
...
const gchar *obex_transfer_get_name(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Name", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
...
const gchar *obex_transfer_get_session(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Session", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
...
guint64 obex_transfer_get_size(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Size", error);
	if(prop == NULL)
		return 0;
...
const gchar *obex_transfer_get_status(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Status", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
...
guint64 obex_transfer_get_time(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Time", error);
	if(prop == NULL)
		return 0;
	guint64 ret = g_variant_get_uint64(prop);
	g_variant_unref(prop);
...
guint64 obex_transfer_get_transferred(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Transferred", error);
	if(prop == NULL)
		return 0;
	guint64 ret = g_variant_get_uint64(prop);
	g_variant_unref(prop);
...
const gchar *obex_transfer_get_transfer_type(ObexTransfer *self, GError **error)
{
	g_assert(OBEX_TRANSFER_IS(self));
	g_assert(self->priv->properties != NULL);
	GVariant *prop = g_proxy_properties_get(self->priv->properties, OBEX_TRANSFER_DBUS_INTERFACE, "Type", error);
	if(prop == NULL)
		return NULL;
	const gchar *ret = g_variant_get_string(prop, NULL);
	g_variant_unref(prop);
